...
public class Matrice {
		...
		for(int i=0;i<n;i++){
			for(int j=0;j<n;j++){
				for(int k=0;k<n;k++){
					...
					tmp.maMatrice[i][j]=tmp.maMatrice[i][j]+maMatrice[i][k]*mPourProduit.maMatrice[k][j];
	...
	public void sommeMatrice(Matrice mPourSomme){
		...
		for(int i=0;i<n;i++){
			for(int j=0;j<n;j++){
				tmp.maMatrice[i][j]=tmp.maMatrice[i][j]+maMatrice[i][j]+mPourSomme.maMatrice[i][j];
			}
		}
		maMatrice=tmp.maMatrice;
